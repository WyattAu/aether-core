...
class SchemaRegistry(ABC):
    ...
    @abstractmethod
    async def validate(
        self, name: str, data: Any, version: Optional[str] = None
    ) -> bool:
        """
        Validate data against a schema.
...
class InMemorySchemaRegistry(SchemaRegistry):
    ...
    async def register(
        ...
        async with self._lock:
            ...
            if existing_versions:
                ...
                if (
                    computed_compat == Compatibility.NONE
                    and compatibility != Compatibility.NONE
                ):
    ...
    async def validate(
        ...
        if errors:
            raise SchemaError(f"Validation failed: {'; '.join(errors)}", name)
