...
import threading
import weakref
from ctypes import create_string_buffer, string_at, memmove
from array import array

from .types import StreamEvent, Timestamp
...
class MemoryPool:
    ...
    def _allocate_batch(self, count: int) -> None:
        ...
        with self._lock:
            ...
            self._stats.total_bytes_allocated = self._stats.total_buffers * self.buffer_size
    ...
    def shrink(self, target_count: int) -> int:
        ...
        with self._lock:
            released = 0
            while len(self._available) > target_count and len(self._all_buffers) > target_count:
                buffer = self._available.pop()
...
class ZeroCopyBuffer(Generic[T]):
    ...
    def append_event(self, event: StreamEvent[T], serializer: Callable[[T], bytes]) -> bool:
        """
