...
class AIRequest:
    ...
    def __init__(self, prompt: str, model: str = "aether-1.0", 
...
class AIActor(Actor):
    ...
    async def on_start(self) -> None:
        ...
        logger.info(f"[{self.name}] Capabilities: AI inference, text generation, embeddings")
    
    async def on_stop(self) -> None:
        logger.info(f"[{self.name}] AI Actor stopping. Total requests: {self.request_count}")
