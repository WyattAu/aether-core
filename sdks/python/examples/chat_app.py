...
class User:
    ...
    def to_dict(self) -> dict:
        return {"user_id": self.user_id, "name": self.name, "joined_at": self.joined_at}
...
class RoomActor(Actor):
    ...
    async def on_start(self) -> None:
        ...
        if data:
            try:
                ...
                logger.info(
                    f"[{self.name}] Restored {len(self.users)} users, {len(self.messages)} messages"
                )
    ...
    async def _handle_join(self, payload: dict) -> Message:
        ...
        logger.info(
            f"[{self.name}] User '{user_name}' joined (total: {len(self.users)})"
        )
    ...
    async def _handle_leave(self, payload: dict) -> Message:
        ...
        if user:
            await self._save_state()
            logger.info(
                f"[{self.name}] User '{user.name}' left (remaining: {len(self.users)})"
            )
...
class SessionActor(Actor):
    ...
    async def handle_message(self, sender: str, message: Message) -> Message | None:
        ...
        if action == "status":
            ...
            logger.info(
                f"[{self.name}] Joined room '{room_id}' (total: {len(self.rooms)})"
            )
            return Message.response(
                {
                    "action": "joined_room",
            ...
            logger.info(
                f"[{self.name}] Left room '{room_id}' (remaining: {len(self.rooms)})"
            )
            return Message.response(
                {
                    "action": "left_room",
...
class ChatApp:
    ...
    async def start(self) -> None:
        ...
        await self.session_actor.handle_message(
            "system", Message.request({"action": "join_room", "room_id": "general"})
        )

        await self.room_actor.handle_message(
            "system",
            Message.request(
                {"action": "join", "user_id": "demo-user", "user_name": "Demo User"}
            ),
        )
    ...
    async def demo(self) -> None:
        ...
        # Get room info
        if self.room_actor:
            info = await self.room_actor.handle_message(
                "demo-user", Message.request({"action": "info"})
            )
        ...
        # Get message history
        if self.room_actor:
            history = await self.room_actor.handle_message(
                "demo-user", Message.request({"action": "history", "limit": 10})
            )
