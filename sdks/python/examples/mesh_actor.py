...
class MeshNode:
    ...
    def __init__(self, node_id: str, region: str, endpoint: str, status: str = "active"):
...
class MeshMessage:
    ...
    def __init__(self, source_node: str, content: str, target_node: str = None, hop_count: int = 0):
...
class MeshActor(Actor):
    ...
    async def on_stop(self) -> None:
        ...
        logger.info(f"[{self.node_id}] Known nodes: {len(self.known_nodes)}, Messages: {len(self.message_log)}")
...
async def main():
    ...
    logger.info(f"Starting mesh actor...")
    logger.info(f"Node ID: {actor.node_id}")
    logger.info(f"Region: {actor.region}")
    logger.info("Supported actions: ping, discover, broadcast, direct_message, get_status, elect_leader")
