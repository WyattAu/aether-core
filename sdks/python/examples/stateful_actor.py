...
from aether_sdk import Actor, Capability, Message, MessageType
    ...
    async def handle_message(self, sender: str, message: Message) -> Optional[Message]:
        if message.type == MessageType.CUSTOM:
            ...
            if action == "increment":
                ...
                return Message(type=MessageType.CUSTOM, payload={"counter": count})
                ...
                return Message(type=MessageType.CUSTOM, payload={"counter": 0})
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        for i in range(3):
            msg = Message(type=MessageType.CUSTOM, payload={"action": "increment"})
            response = await actor.handle_message("test", msg)
            print(f"After increment {i+1}: counter = {response.payload['counter']}")

        msg = Message(type=MessageType.CUSTOM, payload={"action": "get"})
        response = await actor.handle_message("test", msg)
