...
class TestActorClass:
    ...
    async def test_actor_process_item_send(self):
        ...
        await actor._mailbox.put(("send", "target", Message(type=MessageType.CUSTOM, payload="test")))
        
        # Process the item
        await actor._process_item(("send", "target", Message(type=MessageType.CUSTOM, payload="test")))
