...
from aether_sdk.event.pubsub import (InMemoryPubSub, PubSubClient,
                                     PubSubMessage, Subscription, Topic,
                                     publish, subscribe)
...
class TestInMemoryPubSub:
    ...
    async def test_publish_batch(self):
        ...
        messages = [PubSubMessage(topic="test", value=f"msg{i}") for i in range(3)]
